...
Citizen.CreateThread(function()
  ...
  QB_Player = QBCore.Functions.GetPlayerData()
end)
...
AddEventHandler('devcore_smokev2:client:Smoke', function(item, size, prop, type, time)
	if StartSmoke == true then
		exports['mythic_notify']:DoHudText('inform', 'already_smoke')
	else
		...
		hand = true
			...
			while true do
				...
				if StartSmoke then
					...
					if hand then
				if give then
				TriggerClientEvent('QBCore:Notify', src, 'give', 'success')
				...
				if type == 'vape' then
					TriggerClientEvent('QBCore:Notify', src, 'vape', 'success')
					else
				if type == 'bong' then
					TriggerClientEvent('QBCore:Notify', src, 'bong', 'success')
					else
					TriggerClientEvent('QBCore:Notify', src, 'hand', 'success')
				end
			end
		end
	end
				if mouth then
					TriggerClientEvent('QBCore:Notify', src, 'mouth')
				end
		...
		if IsEntityInWater(playerPed) then
			...
			if not give and IsControlJustPressed(0, Config.SmokeButton) then
				if multiplier >= 0.9009426647617 then
					...
					exports['mythic_notify']:DoHudText('inform', 'enough')
					end
				else
				if hand then
				...
				if type == 'bong' or type == 'vape' then
					if size < 0.5 then
						size = 0
						exports['mythic_notify']:DoHudText('inform', 'empty')
						hand = true
			...
			elseif not give and hand and IsControlJustPressed(0, Config.MouthButton) then
				if type == 'vape' then
					if size >= Config.MaxLiquid then
						exports['mythic_notify']:DoHudText('inform', 'nospace')
					else
					...
					if type == 'bong' then
						if size >= Config.MaxWeed then
							...
							for k, v in pairs(Config.BongReloadItems) do
								...
								QBCore.UI.Menu.Open('default', GetCurrentResourceName(), 'drug', {
									title    = "menu_title",
									align    = 'right',
									elements = elements
								}, function(data, menu)
		...
		hand = true
			...
			while true do
				...
				if StartSmoke then
					...
					if hand then
				if give then
					TriggerClientEvent('QBCore:Notify', src,'give')
				local closestPlayer, closestDistance = QBCore.Functions.GetClosestPlayer()
				if closestPlayer ~= -1 and closestDistance <= 2.5 then
					target_id = GetPlayerPed(closestPlayer)
					playerX, playerY, playerZ = table.unpack(GetEntityCoords(target_id))
				...
				if type == 'vape' then
					TriggerClientEvent('QBCore:Notify', src,'vape')
					else
				if type == 'bong' then
					TriggerClientEvent('QBCore:Notify', src,'bong')
					else
						TriggerClientEvent('QBCore:Notify', src, 'hand')
				end
			end
		end
	end
